...
[**{Curso de HTML basico}**](https://github.com/alejanCodeGF/Curso-HTML)
[**{Colores hexadecimales}**](https://htmlcolorcodes.com/)
[**{2 colores para que queden bien complementados}**](https://randoma11y.com/)
[**{Pagina de fotografias de iconos}**](https://icon-icons.com/es/buscar/iconos/?filtro=interface+icons&sort=recent&page=1)

Mucho de este curso está recogido de **SoyDalto**, entre otros recursos online (muy recomendable ver el video)
[**{Link del video}**](https://www.youtube.com/watch?v=OWKXEJN67FE&ab_channel=SoyDalto)
...
5.1. "[atributo="valor"]": Selecciona elementos cuyo atributo es exactamente igual al valor
    > (P.e: Si valor = "Hola", si atributo="Holaa", no lo seleccionará)
5.2. "[atributo^=valor]": Selecciona elementos cuyo atributo comienza con el valor especificado
    > (P.e: Si valor = "Hola", si atributo="Holaa", como empieza por "Hola" si lo selecciona)
5.3. "[atributo$=valor]": Selecciona elementos cuyo atributo termina con el valor especificado
5.4. "[atributo*=valor]": Selecciona elementos cuyo atributo contiene el valor especificado en cualquier posición
5.5. "[atributo~=valor]": Selecciona elementos cuyo atributo contiene el valor entremedias, solo si está separado por espacios
    > (P.e: Si valor = "Hola", si atributo="Ey ComoEstas Hola QueTal", lo seleccionará, porque está separado por espacios)
6. Descendiente ("selector-padre selector-hijo{...}"): Seleccionar etiquetas dentro de otras
...
7. Descendiente directo ("selector-padre > selector-hijo"): Muy parecido, pero SOLO lo selecciona si es hijo directo
    > Es decir, si hay otro selector entre ambos, no lo selecciona, a diferencia del otro que selecciona todos
        > (P.e: <p><a>asd<span>asd</span></a></p>, si hago p > span no lo detecta, porque hay un <a> entre medias, pero p span si)
8. Pseudo-clases: Los eventos de una selección "elemento:acción"
    > (P.e: Pasar el raton por encima hace algo -> p:hover)
8.1. "Selector:first-child": Para de ese grupo (div, lista, etc.), selecciona el primer elemento
8.2. "Selector:last-child": Lo mismo que el first-child, pero al revés, selecciona el ultimo
8.3. "Selector:nth-child(i)": Parecido al first-child, pero selecciona el elemento "i" de la lista de elementos
    > i = 2, 3, 4...
9. Pseudo-elementos: Podemos seleccionar de un selector una parte y editarla (explicado mas adelante)
10. Seleccionar mas de un selector: Poniendo "," entre selectores permite seleccionar varios
    > (P.e: slector1, selector2 {...} le estamos aplicando a los 2 selectores)
...
```
Evitar conflictos, conocer los elementos que seleccionamos a partir de las clases

Si hay elementos en un contenedor, ponerlo de esta forma:
[nombre-class-contenedor__nombre-elementos--nombre-especial]
    > "__" para el bloque y "--" separando los nombres especiales
    > Y así, es mucho mejor y mas organizado cuando vayas a ponerlo en el css

> Si hay un div, que tiene dentro un p que tiene dentro un h2, se pondria con un "-" y ya
    [mi-div__p-h2] (Ya que no consideramos p como "bloque" importante para el h2)
```
...
HTML
    <div class="mi-form">
        <input class="mi-form__mi-input">
        <input class="mi-form__mi-input--el-especial">
    </div>
CSS
    .mi-form__mi-input {...}
    .mi-form__mi-input--el-especial {...}
```
...
Relativas: Se adapta a la pantalla
em -> Será igual al valor de la caja contenedora (de base 16 pixeles), puedes hacer 5em, 7em...
...
```
Un archivo css para poner los estilos por defecto que nosotros queramos y no los del buscador

> Hay distintos modelos de reset, uno de los mas comunes es el normalize
    > Está en la carpeta de "Proyectos"
```
...
Box-sizing:
    ...
    border-box -> Si definimos un height y un widht, será la caja
...
text-shadow -> Sombra del texto, propiedad acortada
...
position:
    static -> Valor por defecto, NO ESTA POSICIONADO
    relative
        > Posicionamos la caja y ponemos el espacio de la caja fijo, y la podemos mover
        > El espacio que dejaba la caja se sigue conservando aunque se mueva
        > Se puede mover ("#Punto de referencia")
    absolut
        > Posicionamos la caja, y le quitamos el espacio que tenia reservado
            > Todo lo de alrededor se reposiciona ocupando el espacio que deja
        > El punto de referecia será el contenedor (top: 0. lo pondrá arriba de todo del contenedor)
            > Si no defines punto de referencia, se queda donde estaba definido al principio
            > Solo se adapta al contenedor (si tiene), si este tiene una posición
            > Ahora el ancho se ajusta al hueco (inline)
    fixed
        > Lo mismo que absolut pero se queda fijo en la pantalla
            > No reserva espacio, poniendose el contenido detrás, etc.
...
background-attachment -> Como se visualiza la imagen
    ...
    fixed -> Si bajas, la imagen está como por detrás de la pantalla fijo a esta
        > Si la imagen está en un recuadro, cuando bajes se verá la misma parte de la imagen, que vas "recortando" mientras bajas
```
...
Limites del tamaño:
min-widht: ...px -> resolución minima
max-widht: ...px -> resolución maxima
...
@media (min-widht: 800px) and (max-widht: 1199px) {...}
    > Entre 800 y 1199 harás estas otras
@media (max-widht: 799px) {...}
    > Menos de 800 harás estas otras cosas
@media screen and (max-width: 400px) {...}
    > Si es una pantalla, y tiene menos de 400px de widht, harás esto
```
...
min-widht / min-height: poner un minimo de tamaño, para que no se haga mas pequeño, pero si mas grande
max-widht / max-height: Al revés del anterior
...
flex-basis -> Basicamente el widht, para los items de flex
...
flex-shrink -> Cuando se deforman las cajas, el espacio que va a ceder comparado con las otras
    ...
    > Cuando miden lo que tienen que medir (widht:...), van a ser iguales, solo cuando las haces mas pequeñas ocurre esto
    > Lo hacen progresivo, van a llegar al widht a la vez todos (solo que augmentan con "velocidad" diferente)
...
> Si volvemos a definir la variable dentro de un selector, cambia para ese selector
```
...
url(link) para cuando quieras poner url de internet
```

## LO ULTIMO

[**{EXPLICANDO PAGINAS WEB POR DENTRO SoyDalto}**](https://youtu.be/OWKXEJN67FE)
